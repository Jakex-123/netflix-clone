...
import { FaPlay, FaVolumeDown, FaVolumeMute, FaVolumeOff, FaVolumeUp } from 'react-icons/fa'
...
function Modal() {
  ...
  return (
    <MuiModal className="fixed !top-7 left-0 right-0 z-50 mx-auto w-full max-w-5xl overflow-hidden overflow-y-scroll rounded-md scrollbar-hide" onClose={handleClose} open={showModal}>
      <>
        <button onClick={handleClose} className='absolute right-5 top-5 !z-40 modalButton border-none bg-[#181818]'>
          <XMarkIcon className='h-9 w-9' />
        </button>
        <div className='relative pt-[56.25%]'>
          <ReactPlayer url={`https://www.youtube.com/watch?v=${trailer}`} width='100%' height='100%' playing style={{ position: 'absolute', top: '0', left: '0' }} muted={muted} />
